...
class Order:
    ...
    def __init__(self, book_id, customer_id, fname, lname, book_title, book_author, email, quantity, shipping):
    ...
    def calculateTotalPrice(self, unit_price):
        if self._shipping:
            ...
            self._total_price = (price * quantity) + 5.00
        else:
            price = unit_price
            quantity = int(self._quantity)
    ...
    def updateQuantity(self):
        ...
        for book in books:
            if book["ID"] == int(self.__book_id):

                if int(self._quantity) > book["quantity"]:
                    return False
                
                book["quantity"] -= int(self._quantity)
